  ...
  const handleCheckConfig = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setConfigResult(`❌ Problemas de configuración: ${result.error?.message || 'Verificar migraciones'}`);
      }
    } catch (error: any) {
      setConfigStatus('error');
  ...
  const handleCheckStructure = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStructureResult(`❌ Error verificando estructura: ${result.error?.message || 'Error desconocido'}`);
      }
    } catch (error: any) {
      setStructureStatus('error');
