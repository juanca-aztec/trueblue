...
export async function testSupabaseRealTime() {
  try {
    ...
    const { data: rpcData, error: rpcError } = await supabase
      .rpc('get_realtime_enabled_tables');
...
export async function checkRealTimeTables() {
  try {
    ...
    if (pubError) {
      ...
      const hasConversations = publicationInfo?.some(pub => pub.tablename === 'tb_conversations');
      const hasMessages = publicationInfo?.some(pub => pub.tablename === 'tb_messages');
